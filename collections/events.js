...
Meteor.methods({
	...
	//-----------------------------EVENTS CREATION METHOD------------------------------------------//
	 ...
	 addEvent: function(eventAttributes){
		...
		var newEvent = _.extend(_.pick(eventAttributes, 'name', 'description', 'isTimed', 'time'), {
	...
	eventComplete: function(id) {
		...
		Events.update(id, { $inc: { 'numberOfEnrolls': -1, 'numberOfCompleted': 1 }, $set: { 'lastCompletedOn': now, 'lastCompletedBy': profile.userName } } );
	},
